...
columns = ['PM10', 'SO2', 'WSPM', 'hour', 'projection', 'PRES', 'month', 'year', 'day', 'O3', 'RAIN']
...
for i in range(len(df_arr)):
    ...
    model = ts.trainModel_GRU(X_train_ts=x_ts_train, y_train_ts=y_ts_train, window_size=10, num_epochs=200)
    # train data metrics
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # Format: YYYYMMDD_HHMMSS
    folder_name = base_filepath + filepaths[i]
    model.save(f'{folder_name}/model_{timestamp}.h5')
    plot_train = ts.print_metrics_and_plot(model, x_ts_train, y_ts_train, f"{folder_name}/trainmetrics{timestamp}.txt", filepaths[i])
    # test data metric
    plot_test = ts.print_metrics_and_plot(model, x_ts_test, y_ts_test, f"{folder_name}/testmetrics{timestamp}.txt", filepaths[i])
    plot_train.show()
    plot_test.show()
